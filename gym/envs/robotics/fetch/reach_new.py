...
MODEL_XML_PATH = os.path.join('fetch', 'reach_new.xml')

def goal_distance(goal_a, goal_b):
    assert goal_a.shape == goal_b.shape
    return np.linalg.norm(goal_a - goal_b, axis=-1)

class FetchReachNewEnv(robot_env.RobotEnv, gym.utils.EzPickle):
    def __init__(self, reward_type='sparse'):
        ...
        super(FetchReachNewEnv, self).__init__(
    ...
    def render(self, mode='human', width=500, height=500):
        return super(FetchReachNewEnv, self).render(mode, width, height)
